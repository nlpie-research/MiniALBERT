...
ds = load_from_disk("tokenizedDatasets/wikipedia-256") #Path to the tokenized dataset

modelPath = "bert-base-uncased"
tokenizer = ts.AutoTokenizer.from_pretrained(modelPath)
teacher = ts.AutoModelForMaskedLM.from_pretrained("bert-base-uncased")
...
student = MiniAlbertForMaskedLM(config=pretrainedConfig, pretrained_embeddings=ts.AutoModel.from_pretrained("huawei-noah/TinyBERT_General_4L_312D").embeddings)
...
savePath = "miniALBERT/models/general-miniALBERT/"
...
trainingArguments = ts.TrainingArguments(
    ...
    warmup_steps=5000,
    per_gpu_train_batch_size=48, #4 gpus were used, therefore, the total batch size is 192
    weight_decay=1e-4,
    save_total_limit=5,
